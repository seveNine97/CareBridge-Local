...
def _compose_grounded_prompt(question: str, triage: TriageAssessment, citations: list[str]) -> tuple[str, str]:
    system_prompt = (
        ...
        "Return every answer in the exact Markdown section structure requested by the user prompt."
    )
    ...
    user_prompt = (
        ...
        "### Clinical Triage\n"
        "**Recommended level**: [Low risk / Medium risk / High risk]\n"
        "**Primary action**: [one concise sentence]\n\n"
        "### Analysis\n"
        "**Symptom interpretation**:\n"
        "* [point-by-point analysis of key symptoms]\n\n"
        "**Differential considerations**:\n"
        "* [risk directions or diagnostic considerations; do not make a final diagnosis]\n\n"
        "### Reasoning Path\n"
        "* [clear medical reasoning for the classification and recommendation]\n\n"
        "### Evidence-Based Support\n"
        "> This response is based on local knowledge base documents:\n"
        "1. [document/page] - \"short relevant quote\"\n"
        "2. [document/page] - \"short relevant quote\"\n\n"
        "### Disclaimer\n"
        "*Generated by a local offline model for clinical support only. It does not replace human medical judgment or establish a final diagnosis.*"
    )
...
async def chat_stream(payload: ChatRequest) -> StreamingResponse:
    ...
    async def event_stream() -> AsyncGenerator[str, None]:
        ...
        for token in runtime_manager.stream_generate(system_prompt=system_prompt, user_prompt=user_prompt):
            yield f"data: {json.dumps({'type': 'chunk', 'text': token}, ensure_ascii=True)}\n\n"
        yield f"data: {json.dumps({'type': 'runtime', 'runtime': runtime_manager.status_payload()}, ensure_ascii=True)}\n\n"
